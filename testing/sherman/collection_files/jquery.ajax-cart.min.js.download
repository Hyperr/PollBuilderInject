! function($) {
	...
	CartForm.prototype.sendToCartSuccess = function() {
		...
		this.$form.find(".ajax-cart-message").hide(686, function() {
			this.remove()
		})
	...
	ProductForm.prototype.sendToCartSuccess = function(result) {
		var message = result.title + ' successfully added. You have ' + result.quantity + ' in your cart. <a style="color: black;" href="/cart">View cart</a>';
		this._displayMessage(message, "success")
		...
		var $button = this.$form.find('button[name="add"]');
	...
	CartDisplay.prototype.updateCartCountElement = function(count) {
		count > 0 && (0 === this.$cartCountBadge.length && (this.$cartCountElement.append(' <span class="badge"></span>'), this.$cartCountBadge = this.$cartCountElement.find(".badge")), this.$cartCountBadge.text() !== count && this.$cartCountBadge.text(count))
	};
